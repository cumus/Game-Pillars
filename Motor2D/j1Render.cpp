...
bool j1Render::Blit(SDL_Texture* texture, int x, int y, const SDL_Rect section) const
{
	...
	if (section.h > 0 && section.w > 0)
	{
		if (!(ret = (SDL_RenderCopyEx(renderer, texture, &section, &rect, 0, NULL, SDL_FLIP_NONE) == 0)))
		{
	...
	{
		if (!(ret = (SDL_RenderCopyEx(renderer, texture, NULL, &rect, 0, NULL, SDL_FLIP_NONE) == 0)))
		{
			LOG("Cannot blit to screen. SDL_RenderCopy error: %s", SDL_GetError());
		}
	}
